...
def getMean(points):
    count1 = 0
    #duas dimensoes pq estava dando muito trabalho e eu nao tinha tempo para adaptar para N dimensoes
    iSumX = 0.0
    iSumY = 0.0
    for i in points:
        iSumX = iSumX + i.coords[0]
        iSumY = iSumY + i.coords[1]
        count1 = count1 + 1
    return Point([iSumX/count1, iSumX/count1])
...
def kmeans(points, k, cutoff):
    ...
    increments = total_size/k
    initial_centroids = []
    count1 = 0
    while count1 < total_size:
        initial_centroids.append(getMean(points[int(count1): int(count1 + increments)]))
        count1 = count1 + increments
